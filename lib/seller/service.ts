...
function normalizeSellerProduct(raw: unknown) {
  ...
  return {
    ...
    stock: readNumber(record, ["stock", "inventory"], 0),
    expiryDate: readString(record, ["expiryDate", "expiresAt"], ""),
    imageUrl: readNullableString(record, ["imageUrl", "thumbnailUrl"]),
    status: normalizeStatus(readString(record, ["status"], "ON_SALE")),
